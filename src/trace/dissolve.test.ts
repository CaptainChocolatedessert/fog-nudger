...
import { insertEdge, removeEdges } from "./planarOps";
...
/*
  ...
  Two generators feed it, for the reason given on `randomGraph`.
*/
 ...
 *
 * **Not the derivation's generator, and the reason is measured** (2026-09-16). Skeletons of random
 * ink runs gave 510 maps' worth of regions and not one wall kept — linework thinned from pixels almost never puts a
 * closed room inside another, so a sweep over it agreed with the oracle without once reaching the
 * rule this tool exists for. Shapes on an eleven-point lattice nest and share corners, and small
 ...
 */
function randomGraph(next: () => number, shapes: number): WallGraph {
...
describe("dissolving over generated graphs", () => {
  it("agrees with reachability for every region, including inner rooms joined to the outer wall", () => {
    ...
    for (let seed = 1; seed <= seeds; seed++) {
      const graph = randomGraph(rng(seed), 3 + (seed % 9));
      const faces = buildWallFaces(graph);
  ...
  */
  ...
  for (const { width, height, runs, seeds } of SHAPES) {
    it(`agrees with reachability for every region of ${seeds} random ${width}x${height} maps`, () => {
      let regions = 0;
      for (let seed = 1; seed <= seeds; seed++) {
        const { walls, faces } = deriveWalls(randomInk(width, height, rng(seed), runs), {
          tolerance: 0,
          maxTolerance: 0,
  ...
  it("finds the region a point is in, agreeing with the emitted rings' even-odd fill", () => {
    ...
    for (let seed = 1; seed <= 60; seed++) {
      const width = 40;
      const height = 30;
      const { faces } = deriveWalls(randomInk(width, height, rng(seed), 22), {
        tolerance: 0,
        maxTolerance: 0,
        extent: graphExtent(width, height),
      });
