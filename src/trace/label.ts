...
export interface Region {
   ...
   *
   * Reported, never acted on. It used to be the candidate rule for finding the exterior and it was
   * retired for being wrong on any map whose rooms run to the edge — which is common. It survives
   * as information, because a sudden change in how many regions touch the border between two runs
...
export interface LabelOptions {
   ...
   *
   * **Denominate this in grid squares at the call site**, never in raster pixels (DESIGN.md §5).
   * The trade it makes has no free answer: high enough to kill cross-hatching, it eventually eats a
   * genuine closet. Set low, per §5's bias — a spurious extra region costs the GM a click, a
   * missing room costs them a room.
   */
