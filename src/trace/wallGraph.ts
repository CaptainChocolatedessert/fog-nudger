 ...
 *
 * Step G derives here. Stage one is the map — tune the reading, edit pixels, generate the graph.
 ...
 *
 * `decodeWallGraph` returns `null` for anything it cannot read, and never throws. Metadata arrives
 ...
 * legitimate state — it is what a scene looks like before anything was ever saved.
 *
 * Pure: no DOM, no SDK.
 */
...
export interface WallGraphBuild {
   ...
   * collapse it, and says so in the log — and the derivation stores fitted edges with no equivalent.
   *
  ...
  */
  ...
  for (let i = 0; i < graph.edges.length; i++) {
    ...
    /*
      ...
      The derivation is where the document is built, so this is the last moment before anything a GM sees
      — Walls draws the wall graph — and before the emit path turns each remaining segment into its
 ...
 */
export function nodeDegrees(graph: WallGraph): number[] {
 ...
 */
export function compactNodes(graph: WallGraph): WallGraph {
 ...
 */
export function wallRuns(graph: WallGraph): number[][] {
 ...
 */
export function wallRunEdges(graph: WallGraph): number[][] {
...
function walkRuns(graph: WallGraph): TracedRun[] {
 ...
 *
 * Doing it here has neither problem, because **there is no raster left to disturb**. The derivation is
 ...
 * visualization."*
 *
 * `pruneWallGraph` is written in terms of it, so the preview and the operation cannot disagree —
...
export function spurEdgesToPrune(graph: WallGraph, limit: number): DoomedSpurs {
  ...
  // of `pruneWallGraph` decides. Counted from the surviving edges rather than the doomed ones, so
 ...
 * happen to be close.
 */
function runLength(graph: WallGraph, nodes: readonly number[]): number {
  let total = 0;
 ...
 */
export function longestRun(graph: WallGraph): number {
  let longest = 0;
 ...
 * Zero when there is nothing to measure, which the caller reads as "no top".
 */
export function largestBend(graph: WallGraph): number {
  const degrees = nodeDegrees(graph);
...
export function encodeWallGraph(graph: WallGraph): string {
