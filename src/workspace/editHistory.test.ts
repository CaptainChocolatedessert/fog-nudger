...
describe("EditHistory", () => {
  it("takes edits back newest first", () => {
    ...
    expect(h.pop()).toEqual({ document: "b", label: "second" });
    expect(h.pop()).toEqual({ document: "a", label: "first" });
  ...
  it("remembers the state before the edit, not after it", () => {
    ...
    */
    const h = history();
    h.push("before the erase", "erasing a wall");
  ...
  it("says what it would take back without taking it", () => {
    const h = history();
    h.push("a", "pruning the dead ends");
  ...
  it("drops the oldest once it is full, and keeps the bound exactly", () => {
    ...
    const h = history(3);
    for (const n of ["a", "b", "c", "d", "e"]) h.push(n, n);
  ...
  it("forgets everything when the document is replaced", () => {
    ...
    */
    const h = history();
    h.push("a", "drawing a wall");
    h.push("b", "erasing a wall");
    h.clear();
  ...
  it("goes on working after being cleared", () => {
    ...
    h.push("a", "first");
    h.clear();
    h.push("b", "after the re-read");

    expect(h.size).toBe(1);
    expect(h.pop()).toEqual({ document: "b", label: "after the re-read" });
  });
});
